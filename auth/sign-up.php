...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    // Check validation
    if (!$email || empty($password) || !$user_type || empty($name)) {
        array_push($error, "All Fields are required and must be valid!");
    } else {

        // Check User Exist
        $response = $db->getOneDataFromTable("SELECT user_id FROM users WHERE email = $email");

        if ($response['data']) {
            array_push($error, "Email already exists $response");
        } else {
            $time = date("Y-m-d");
            $randId = rand(10000000, 20000000);
            $hashedPassword = password_hash($password, PASSWORD_BCRYPT);
            $res = $db->insertToTable("INSERT INTO users(user_id, user_name, email, password_hash, user_type, created_at, updated_at) VALUES ($randId, $name, $email, $hashedPassword, $user_type, $time, $time)");
            $newUserId = $res['data'];
...
<body class="bg-[url('/job-portal/components/hero-pattern-dark.svg')]">


    <main class="max-w-4xl flex items-center mx-auto md:min-h-screen p-4 md:p-8">
        <div
            class="grid bg-white items-center gap-y-10 bg-[url('/job-portal/components/card-img.jpg')]border border-slate-100 [box-shadow:0_2px_10px_-3px_rgba(14,14,14,0.3)] rounded-lg overflow-hidden md:grid-cols-3">
            <div
            ...
            <div class="w-full py-6 px-6 max-w-lg mx-auto md:col-span-2 md:px-14">
                ...
                <div class="mb-10 flex flex-row gap-3">
                    <a href="/job-portal/index.php" class="mr-5 rounded rounded-xl bg-indigo-700 text-md font-bold text-white px-5 py-2">Home</a>
                ...
                <div class="mt-6 text-slate-900 text-sm text-center">Already have an account? <a href="/job-portal/auth/sign-in.php"
