...
if (isset($db) && $db instanceof Database) {
    $latestJobs = $db->getAllDataFromTable("SELECT * FROM recent_jobs ORDER BY listed_at DESC LIMIT 3");
    if (is_array($latestJobs) && ($latestJobs['completed'] ?? false)) {
...
<div class="bg-[url('./components/hero-pattern.svg')] bg-indigo-50 text-black md:p-25 p-19 mt-4">
    ...
    <div class="flex flex-col xl:flex-row gap-6 sm:gap-10 justify-center items-center">
        ...
        <?php if (!empty($jobs)): ?>
            <?php foreach ($jobs as $job): ?>
                <div class="flex flex-col items-center justify-center max-h-xl">
                    <div class="bg-indigo-100 block max-w-sm p-6 border border-default rounded-xl shadow-2xl">
                        <a href="#">
                            <img class="rounded-xl h-[300px]" src="<?php echo htmlspecialchars('./' . ($job->job_image ?? '')); ?>" />
                        </a>
                        <a href="#">
                            <h5 class="mt-6 mb-2 text-2xl font-semibold tracking-tight text-heading">
                                <?php echo htmlspecialchars($job->job_title ?? 'Job'); ?>
                            </h5>
                        </a>
                        <p class="mb-6 text-body"><?php echo htmlspecialchars($job->job_description ?? ''); ?></p>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <p class="text-center text-gray-600">No job listings are available right now.</p>
        <?php endif; ?>
